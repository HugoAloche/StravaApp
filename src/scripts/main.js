...
function initApp() {
  ...
  if (urlParams.get('code')) {
    connexionApp(urlParams.get('code'), nom, city, distance, profile, athleteProgress);
  }
}
 ...
 * @param {string} code
 */
function connexionApp(code, nom, city, distance, profile, athleteProgress) {
    ...
    .then(response => {
        response.json().then((data) => {
          ...
          getInformation(data.athlete.id, data.access_token, nom, city, distance, profile, athleteProgress);
        })
    })
    .catch(error => {
        console.log(error)
 ...
 * @param {string} token
 */
function getInformation(id, token, nom, city, distance, profile, athleteProgress) {
  ...
  .then(response => {
      response.json().then((data) => {
        ...
        setInformation(nom, city, distance, profile, athleteProgress)
 ...
 */
function setInformation(name, city, distance, profile, athleteProgress) {
    name.innerHTML = `${athlete.firstname} ${athlete.lastname}`
    city.innerHTML = `${athlete.city}, ${athlete.state}`
